...
const demonstratePromptStructure = () => {
  ...
  // 系统提示词（固定）
  const systemPrompt = `你是一位资深的人生规划顾问，精通基于传统智慧的性格分析方法。你需要根据用户提供的完整八字信息，生成一份专业、详细、全面的个人成长报告。
...
请确保内容专业、准确、有指导价值，基于提供的所有八字数据进行全面分析。`
  ...
  const userPrompt = `请根据以下完整的八字信息和当前时间，生成一份详细的个人成长报告，字数控制在8000字左右：
