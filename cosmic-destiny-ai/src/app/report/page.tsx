...
We are confirming your payment information. This usually takes a few seconds. The page will refresh automatically.`
    ...
    // For unpaid users, extract preview from full_report (same as streaming behavior)
    if (!report.is_paid && report.full_report) {
      // Extract first 3500 characters, same as streaming content preview
      const preview = report.full_report.substring(0, PREVIEW_BOUNDARY)
      return preview
    }
    
    // If there is a full report and user has paid, show full report
    if (report.full_report) {
      return report.full_report
    }
    
    // Fallback: if there is a preview report (for older reports), use it
